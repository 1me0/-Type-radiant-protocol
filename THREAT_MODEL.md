...
| **Replay attacks** | Nonce tracking per user + signature verification | The blueprint is a replayed transaction; but each proof includes a unique nonce, making replay impossible. |
| **Oracle manipulation** | Multi‑source agreement + cross‑source weighting | The blueprint is a single corrupted oracle; the protocol uses at least three independent sources, and a majority vote (weighted by trust) is required. |
...
**Version:** 2.0  
**Audited by:** Radiant Protocol Contributors
